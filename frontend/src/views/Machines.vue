<template>
  <div class="max-w-5xl">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <h1 class="text-xl font-semibold text-gray-900">Máquinas</h1>
      <button
        type="button"
    ...
    <div class="mb-4">
      <div class="relative">
        ...
        <input
          ...
          class="w-full rounded-lg border border-gray-300 py-2.5 pl-10 pr-4 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:max-w-xs"
          @input="onSearchInput"
        />
      </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
          <thead class="bg-gray-50">
            <tr>
              ...
              <th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">Costo/h</th>
              <th class="px-5 py-3 text-left text-xs font-medium uppercase text-gray-500">Activa</th>
          ...
          <tbody class="divide-y divide-gray-200 bg-white">
            <tr v-for="row in items" :key="row.name" class="hover:bg-gray-50">
              <td class="px-5 py-3 font-medium text-gray-900">{{ row.machine_name || row.name }}</td>
              <td class="px-5 py-3 text-sm text-gray-600">{{ row.machine_type || "—" }}</td>
              <td class="px-5 py-3 text-sm text-gray-600">{{ row.cost_per_hour != null ? row.cost_per_hour : "—" }}</td>
              <td class="px-5 py-3 text-sm">{{ row.active ? "Sí" : "No" }}</td>
            ...
            <tr v-if="!loading && items.length === 0">
              <td colspan="5" class="px-5 py-10 text-center text-sm text-gray-500">No hay máquinas.</td>
            </tr>
          </tbody>
        </table>
      </template>
    ...
    <Teleport to="body">
      ...
      >
        <div class="w-full max-w-lg rounded-xl border border-gray-200 bg-white p-6 shadow-xl">
          <h2 class="mb-4 text-lg font-semibold text-gray-900">{{ editingName ? 'Editar máquina' : 'Nueva máquina' }}</h2>
          <form class="space-y-4" @submit.prevent="save">
            ...
            <div>
              <label class="mb-1 block text-sm font-medium text-gray-700">Costo por hora</label>
              <input v-model.number="form.cost_per_hour" type="number" step="0.01" class="w-full rounded-lg border border-gray-300 py-2.5 px-4 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            <div>
              <label class="mb-1 block text-sm font-medium text-gray-700">Tiempo setup (min)</label>
              <input v-model.number="form.setup_time_minutes" type="number" class="w-full rounded-lg border border-gray-300 py-2.5 px-4 text-sm focus:border-indigo-500 focus:outline-none focus:ring-2 focus:ring-indigo-500" />
            </div>
            ...
            <div class="flex justify-end gap-3 pt-2">
              ...
              <button type="submit" :disabled="saving" class="rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-indigo-700 disabled:opacity-50">
...
async function fetchList() {
  loading.value = true;
  try {
    const res = await call("mathipe_ui.api.list_machines", { search: searchTerm.value.trim() || undefined });
...
function openModal(row = null) {
  editingName.value = row ? (row.name || row.machine_name) : null;
  form.machine_name = row ? (row.machine_name || row.name) : "";
  form.machine_type = row ? row.machine_type : "";
  form.sector_key = row ? row.sector_key : (sectors.value[0]?.name || "");
  form.cost_per_hour = row != null && row.cost_per_hour != null ? row.cost_per_hour : null;
  form.setup_time_minutes = row != null && row.setup_time_minutes != null ? row.setup_time_minutes : null;
...
async function save() {
  ...
  try {
    const payload = {
      ...
      setup_time_minutes: form.setup_time_minutes,
      active: form.active,
    };
    if (editingName.value) payload.name = editingName.value;
    await call("mathipe_ui.api.save_machine", { payload });
...
function confirmDelete(row) {
  ...
  const name = row.name || row.machine_name;
  call("mathipe_ui.api.delete_machine", { name })
    .then(() => {
      showToast("Eliminada.");
      fetchList();
    })
